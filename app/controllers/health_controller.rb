class HealthController < ApplicationController
  ...
  # Basic health check - public for load balancers/monitoring
  def index
    # Just check if the app is running and can connect to the database
    db_status = ActiveRecord::Base.connection.active? rescue false

    render json: {
      status: db_status ? "ok" : "error",
      environment: Rails.env,
      timestamp: Time.current
    }, status: db_status ? :ok : :service_unavailable
  end
