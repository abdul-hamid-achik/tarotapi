namespace :llm do
  ...
  desc "install llama_cpp gem for ARM architecture"
  task :install_gem do
    puts "Installing llama_cpp gem for ARM architecture..."

    # Check if llama.cpp is installed
    unless File.exist?("/usr/local/lib/libllama.dylib") || File.exist?("/usr/local/lib/libllama.so")
      puts "Building and installing llama.cpp first..."
      Rake::Task["dev:install_llama_cpp"].invoke
    end

    # Set up environment for gem installation
    env_vars = {
      "LIBRARY_PATH" => "#{ENV['LIBRARY_PATH']}:/usr/local/lib",
      "LD_LIBRARY_PATH" => "#{ENV['LD_LIBRARY_PATH']}:/usr/local/lib",
      "CPATH" => "#{ENV['CPATH']}:/usr/local/include",
      "LLAMA_CPP_LIB" => "/usr/local/lib",
      "LLAMA_CPP_INCLUDE" => "/usr/local/include/llama"
    }

    # Install the gem with environment variables
    puts "Installing llama_cpp gem with proper environment variables..."
    env_string = env_vars.map { |k, v| "#{k}='#{v}'" }.join(" ")
    system("#{env_string} gem install llama_cpp") || abort("Failed to install llama_cpp gem")

    puts "Updating bundle..."
    system("#{env_string} bundle install") || abort("Failed to run bundle install")

    puts "llama_cpp gem installed successfully!"
  end
end
