  ...
  namespace :models do
    ...
    task :run, [:model_name] => :environment do |t, args|
      ...
      if system("bundle exec rspec spec/models/#{model_name}_spec.rb")
        puts "#{model_name} model tests passed"
      else
        puts "#{model_name} model tests failed"
        exit 1
      end
    end
  end
  ...
  namespace :services do
    ...
    task :all do
      if system("bundle exec rspec spec/services")
        puts "all service tests passed"
      else
        puts "some service tests failed"
        exit 1
      end
    end
    ...
    task :verify_coverage do
      ...
      if missing.empty?
        ...
        puts "warning: missing specs for services:"
        missing.each { |svc| puts "  - #{svc}" }
      end
    end
